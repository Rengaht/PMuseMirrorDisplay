...
void main()
{
	...
	//vec2 offset=vec2(cos(texCoordVarying.x*texCoordVarying.y*phi)/windowWidth*phi,sin(texCoordVarying.y*10.0+phi)/windowHeight*2.0);	
	vec2 offset=vec2(0);
	if(cnoise(texCoordVarying)>.5) offset=vec2(phi,phi);
